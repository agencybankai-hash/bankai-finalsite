...
### Типографика — TikTok Sans (заголовки) + Inter (текст), fluid `clamp()`, иерархия размером+трекингом

Заголовки `h1`–`h6` идут на `--font-display` (TikTok Sans, вариативный, с кириллицей), остальной текст — на `--font-sans` (Inter). Оба подключены через `next/font` и самохостятся.
