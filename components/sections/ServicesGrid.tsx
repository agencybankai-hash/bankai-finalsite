...
export function ServicesGrid({ cards }: { cards: ServiceCard[] }) {
  return (
    <Reveal stagger className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {cards.map((c) => {
        // эмблема темы страницы, куда ведёт карточка; нет - общая иконка
        const emblem = emblemForHref(c.href);
        return (
          <Link
            key={c.href}
            href={c.href}
          ...
          >
            {emblem && hasEmblem(emblem.name) ? (
              <Emblem name={emblem.name} geo={emblem.geo} size="lg" className="mb-5" />
            ) : (
