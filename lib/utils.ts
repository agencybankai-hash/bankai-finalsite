...
const twMerge = extendTailwindMerge({
  extend: {
    classGroups: {
      "font-size": [
        { text: ["display", "h1", "hero", "h2", "h3", "lead", "label", "stat"] },
      ],
    },
  },
});
...
// Число целиком: разряды «1 265», дробь «1,6», диапазон «1-2».
const NUM = String.raw`(?:\d{1,3}(?:[ \u00A0]\d{3})+|\d+(?:[.,]\d+)?)`;
// Отдельное число перед словом. Слева - начало строки, скобка или пробел
// не после цифры: хвост разрядов («265» в «1 265») числом не считается,
// иначе строка рвётся внутри числа. Справа - только обычные пробелы:
// перевод строки держит разметку whitespace-pre-line.
const NUMBER_WORD = new RegExp(String.raw`(^|\D\s|\()(${NUM}(?:-${NUM})?) +(?=\p{L})`, "gu");

/**
 * Типографика: ставит неразрывные пробелы после коротких предлогов/союзов,
 * привязывает тире к предыдущему слову, а число - к следующему, чтобы они
 * не отрывались на следующую строку.
 */
export function nbsp(text: string): string {
  ...
  // число прилипает к следующему слову вместе с разрядами: «4<nb>шага», «1<nb>265<nb>заявок»
  return out.replace(
    NUMBER_WORD,
    (_, left: string, num: string) => left + num.replace(/ /g, NB) + NB,
  );
}
